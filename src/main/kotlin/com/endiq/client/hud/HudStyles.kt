...
object HudStyles {
    ...
    fun text(ctx:GuiContext,module:Module,raw:String,fallbackX:Int=2,fallbackY:Int=2) {
        ...
        val lines=value.split('\n');val width=lines.maxOf { font.getWidth(it) };val height=lines.size*10
        ...
        val y=(slider("Position Y",fallbackY*100f/sh)*sh/100f).coerceIn(0f,(sh-height*scale).coerceAtLeast(0f))
        ...
        ctx.transformed(x,y,scale) {
            if(bool("Show Background",false))Theme.rounded(ctx,UiRect(-pad,-pad,width+pad*2,height+pad*2-2),color(listOf("Background Color","Background"),0x88000000.toInt()),slider("BG Radius",2f).toInt())
            lines.forEachIndexed { i,line -> ctx.drawText(font,line,0,i*10,tint,bool("Text Shadow",true)) }
            ...
            if(module is PingModule && module.showBar.value) {
                ...
                for(i in 0..3)ctx.fill(width+4+i*3,8-i*2,width+6+i*3,10,if(i<level)tint else Theme.SUBTLE)
            }
        }
    }
}
